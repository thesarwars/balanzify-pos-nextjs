'use client';
// ─────────────────────────────────────────────────────────────────
// Items Report — one row per sold line item: the sell side in full,
// the line's actual consumed cost, and an indicative purchase source.
// ─────────────────────────────────────────────────────────────────
...
export function ItemsReport({ T }: { T: Theme }) {
  ...
  const cols: ReportCol[] = React.useMemo(() => [
    ...
    { key: 'purchase_date', label: 'Purchase Date', value: (r) => r.purchase_date ? formatDate(r.purchase_date) : '' },
    { key: 'purchase_ref', label: 'Purchase', value: (r) => r.purchase_ref || '' },
    { key: 'lot_number', label: 'Lot Number', value: (r) => r.lot_number || '' },
    { key: 'supplier', label: 'Supplier', value: (r) => r.supplier || '' },
    { key: 'purchase_value', label: 'Purchase Price', kind: 'money', total: true, value: (r) => r.purchase_value },
    { key: 'sell_date', label: 'Sell Date', value: (r) => r.sell_date ? formatDate(r.sell_date) : '' },
    { key: 'sale_ref', label: 'Sale', value: (r) => r.sale_ref || '' },
    { key: 'customer', label: 'Customer', value: (r) => r.customer || '' },
    { key: 'location', label: 'Location', value: (r) => r.location || '' },
    { key: 'sell_quantity', label: 'Sell Quantity', kind: 'num', total: true, value: (r) => r.sell_quantity },
    { key: 'selling_price', label: 'Selling Price', kind: 'money', value: (r) => r.selling_price },
        ...
        <Panel T={T} pad>
          <div style={{ opacity: loading ? 0.6 : 1 }}>
            <ReportTable
              ...
              title={`Items Report — ${periodLabel}`}
              subtitle={periodLabel}
              fileName={`items-${filters.from}-to-${filters.to}`}
              empty={loading ? 'Loading…' : 'No data available in table'}
              note={data?.limited ? <span>Showing the first 5,000 items</span> : <span>Purchase columns show the product&apos;s most recent purchase for context; Purchase Price is the line&apos;s actual consumed cost.</span>}
            />
          </div>
        </Panel>
      )}
