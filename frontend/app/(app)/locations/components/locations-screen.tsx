...
import { SchemeManagerModal } from '@/components/invoice-schemes';
...
export function Locations({ T }: { T: any }) {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: T.paperAlt }}>
      ...
      {schemeMgr && <SchemeManagerModal T={T} schemes={refs.schemes} onClose={() => setSchemeMgr(false)} onChange={loadRefs} toast={show} />}
      {node}
    </div>
  );
}
