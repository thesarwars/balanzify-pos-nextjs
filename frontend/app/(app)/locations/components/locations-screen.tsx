...
const BLANK_SCHEME = { id: null, name: '', prefix: '', start_number: 1, total_digits: 4, numbering_type: 'sequential', include_year: false };

function schemeSample(s: any, year: number) {
...
function SchemeManager({ T, schemes, onClose, onChange, toast }: { T: any; schemes: any; onClose: () => void; onChange: () => void; toast: (m: string) => void }) {
  const [f, setF] = useStateLo<any>({ ...BLANK_SCHEME });
  const [busy, setBusy] = useStateLo(false);
  const set = (k: any, v: any) => setF((s: any) => ({ ...s, [k]: v }));
  const editing = !!f.id;
  ...
  return (
    <Modal T={T} title={editing ? 'Edit invoice scheme' : 'Invoice schemes'} subtitle="Invoice number formats" width={560} onClose={onClose} footer={null}>
      {schemes.length > 0 && (
