...
const bad = (message, statusCode = 400, code) => Object.assign(new Error(message), { statusCode, code });
 ...
 */
function computeTotals({ lines, discountType, discountValue, orderTaxRate, shippingCharges, expenses }) {
  let subtotal = 0;
  let lineTax = 0;
  for (const l of lines) {
    l.net = round2(l.quantity * l.unitPrice - l.lineDiscount);
    if (l.net < 0) throw bad(`Discount on "${l.name}" is larger than the line total.`);
  ...
  const discountAmount = Math.min(subtotal, discountType === 'flat'
  ...
  const expensesTotal = round2(expenses.reduce((s, e) => s + round2(e.amount), 0));
  const total = round2(goods + tax + shipping + expensesTotal);

  return { subtotal, discountAmount, goods, lineTax, orderTax, tax, shipping, expensesTotal, total };
}
 ...
 */
async function consumeStock(tx, { businessId, userId, locationId, productId, variantId, stockQty, fallbackCost, productName, saleId }) {
  ...
  if (onHand < stockQty) {
    throw bad(`Not enough stock for "${productName}" — ${onHand} on hand, ${stockQty} needed.`);
  }
  ...
  if (affected === 0) throw bad(`Stock for "${productName}" changed while the sale was being recorded — please retry.`, 409);
  ...
  for (const layer of layers) {
    ...
    if (dec === 0) throw bad('A cost layer changed while the sale was being recorded — please retry.', 409);
  ...
  await tx.stockMovement.create({
    data: {
      businessId, productId, locationId, type: 'sale',
      quantity: -stockQty, balanceAfter: onHand - stockQty,
      referenceType: 'sale', referenceId: saleId || null, createdById: userId || null,
    },
  });

  return { cogs: round2(cost), unitCost: stockQty > 0 ? cost / stockQty : fallbackCost };
}
...
async function resolveLines(tx, { businessId, items }) {
  ...
  const products = await tx.product.findMany({
    ...
    select: { id: true, name: true, sku: true, sellingPrice: true, costPrice: true, sellByUnit: true, packSize: true },
  });
  const byId = new Map(products.map((p) => [p.id, p]));
  if (byId.size !== productIds.length) throw bad('One of those products does not exist.', 404);
  ...
  await assertNoRecipes(tx, productIds.map((id) => ({ productId: id, name: (byId.get(id) || {}).name })));

  const rateIds = [...new Set(items.map((i) => i.tax_rate_id).filter(Boolean))];
  const rates = rateIds.length
    ? await tx.taxRate.findMany({ where: { id: { in: rateIds }, businessId, isActive: true }, select: { id: true, rate: true } })
    : [];
  const rateById = new Map(rates.map((r) => [r.id, parseFloat(r.rate)]));
  if (rateById.size !== rateIds.length) throw bad('One of those tax rates does not exist.');

  return items.map((i) => {
    const p = byId.get(i.product_id);
    const unitPrice = i.unit_price != null ? round2(i.unit_price) : parseFloat(p.sellingPrice);
    if (unitPrice < 0) throw bad(`Unit price for "${p.name}" cannot be negative.`);
    return {
      productId: p.id,
      ...
      stockQty: (p.sellByUnit && p.packSize) ? Number(i.quantity) * p.packSize : Number(i.quantity),
...
async function assertNoRecipes(tx, lines) {
  ...
  const recipes = await tx.recipe.findMany({ where: { productId: { in: ids } }, select: { productId: true } });
  if (!recipes.length) return;
  const line = lines.find((l) => l.productId === recipes[0].productId);
  throw bad(`"${line ? line.name : 'That product'}" is made to order. Sell it from the Point of Sale, which depletes its ingredients.`);
}
...
async function assertPaymentAccounts(tx, businessId, tenders) {
  const ids = [...new Set(tenders.map((t) => t.payment_account_id).filter(Boolean))];
  if (!ids.length) return;
  const found = await tx.paymentAccount.findMany({ where: { id: { in: ids }, businessId }, select: { id: true } });
...
function resolveTenders(payments, total) {
  ...
  if (amountPaid - total > 0.01) {
    throw bad(`Payments (${amountPaid.toFixed(2)}) exceed the invoice total (${total.toFixed(2)}).`);
  }
 ...
 */
async function postFinal(tx, { businessId, userId, sale, lines, totals, tenders, amountPaid, amountDue, currency }) {
  if (amountDue > 0 && !sale.customerId) {
    throw bad('An unpaid balance has to be billed to a customer — pick one, or take the full amount now.');
  }
  ...
  for (const line of lines) {
    const { cogs: lineCogs, unitCost } = await consumeStock(tx, {
      businessId, userId, saleId: sale.id, locationId: sale.locationId,
      productId: line.productId, variantId: line.variantId,
      stockQty: line.stockQty, fallbackCost: line.fallbackCost, productName: line.name,
    });
    cogs = round2(cogs + lineCogs);
    await tx.saleItem.update({ where: { id: line.saleItemId }, data: { costPrice: round2(unitCost) } });
  }

  let changeGiven = 0;
  for (const t of tenders) {
    const change = t.tendered != null ? Math.max(0, round2(t.tendered - t.amount)) : null;
    if (change) changeGiven = round2(changeGiven + change);
    await tx.salePayment.create({
      data: {
        businessId, saleId: sale.id, provider: t.method, amount: round2(t.amount),
        currency, status: 'completed', note: t.note || null,
  ...
  if (amountDue > 0) {
    await credit.postDebit(tx, {
      businessId, customerId: sale.customerId, amount: amountDue, currency,
      saleId: sale.id, description: `Invoice ${sale.saleNumber || ''}`.trim(), recordedById: userId,
    });
  }

  await accounting.postInvoiceSale(tx, {
    businessId, sale, tenders,
    goods: totals.goods, shipping: totals.shipping, otherIncome: totals.expensesTotal,
    taxAmount: totals.tax, cogs, amountDue, createdById: userId,
  });
  ...
  const sumOf = (...ms) => round2(tenders.filter((t) => ms.includes(t.method)).reduce((s, t) => s + round2(t.amount), 0));
  const distinct = [...new Set(tenders.map((t) => t.method))];
  const paymentMethod = distinct.length === 0 ? 'credit' : distinct.length > 1 ? 'split' : distinct[0];
  ...
  return tx.sale.update({
    ...
    data: {
      ...
      zaadAmount: sumOf('zaad', 'evc', 'edahab', 'mpesa', 'telebirr', 'cbe_birr', 'mobile_money'),
...
async function createInvoice(tx, { businessId, userId, currency = 'USD', body }) {
  const status = body.status || 'completed';
  const isDraft = NON_POSTING.has(status);

  const location = await tx.location.findFirst({ where: { id: body.location_id, businessId }, select: { id: true, invoiceSchemeId: true } });
  if (!location) throw bad('Location not found.', 404);
  if (body.customer_id) {
    const c = await tx.customer.findFirst({ where: { id: body.customer_id, businessId }, select: { id: true } });
    if (!c) throw bad('Customer not found.', 404);
  }
  if (body.delivery_person_id) {
    const u = await tx.user.findFirst({ where: { id: body.delivery_person_id, businessId }, select: { id: true } });
    if (!u) throw bad('Delivery person not found.', 404);
  }

  let orderTaxRate = 0;
  if (body.tax_rate_id) {
    const r = await tx.taxRate.findFirst({ where: { id: body.tax_rate_id, businessId, isActive: true }, select: { rate: true } });
  ...
  const expenses = (body.expenses || []).filter((e) => e.name && round2(e.amount) > 0);
  ...
  if (saleNumber) {
    const clash = await tx.sale.findFirst({ where: { businessId, saleNumber }, select: { id: true } });
    if (clash) throw bad(`Invoice number "${saleNumber}" is already used.`);
  } else if (isDraft) {
    saleNumber = `${status.slice(0, 3).toUpperCase()}-${Date.now()}`;
  } else {
    saleNumber = schemeId ? await mintInvoiceNumber(tx, businessId, schemeId) : `INV-${Date.now()}`;
  }
  ...
  const sale = await tx.sale.create({
    data: {
      ...
      cashierId: userId,
      expenses: { create: expenses.map((e) => ({ name: e.name.trim(), amount: round2(e.amount) })) },
    },
  });
  ...
  for (const l of lines) {
    const item = await tx.saleItem.create({
      data: {
        saleId: sale.id, productId: l.productId, variantId: l.variantId, quantity: l.quantity,
        unitPrice: l.unitPrice, originalPrice: l.unitPrice, discount: l.lineDiscount,
        taxRateId: l.taxRateId, taxAmount: l.taxAmount, totalPrice: l.net, costPrice: 0,
      },
    });
  ...
  if (isDraft) {
    return tx.sale.findUnique({ where: { id: sale.id }, include: { items: true, expenses: true, payments: true } });
  }
  return postFinal(tx, { businessId, userId, sale, lines, totals, tenders, amountPaid, amountDue, currency });
}

/** Turn a draft / quotation / proforma into a real, posted sale. */
async function finalizeInvoice(tx, { businessId, userId, currency = 'USD', saleId, payments }) {
  const rows = await tx.$queryRaw`SELECT status FROM sales WHERE id = ${saleId}::uuid AND business_id = ${businessId}::uuid FOR UPDATE`;
  if (!rows.length) throw bad('Sale not found.', 404);
  if (!NON_POSTING.has(rows[0].status)) throw bad('Only a draft, quotation or proforma can be finalised.');

  const sale = await tx.sale.findUnique({
    where: { id: saleId },
    include: { items: { include: { product: { select: { name: true, costPrice: true, sellByUnit: true, packSize: true } } } }, expenses: true, location: { select: { invoiceSchemeId: true } } },
  });

  const lines = sale.items.map((it) => ({
    saleItemId: it.id,
    productId: it.productId, variantId: it.variantId, name: it.product.name,
    quantity: it.quantity, unitPrice: parseFloat(it.unitPrice), lineDiscount: parseFloat(it.discount),
    taxRateId: it.taxRateId, taxAmount: parseFloat(it.taxAmount), net: parseFloat(it.totalPrice),
    stockQty: (it.product.sellByUnit && it.product.packSize) ? it.quantity * it.product.packSize : it.quantity,
  ...
  const { tenders, amountPaid, amountDue } = resolveTenders(payments, totals.total);
  ...
  return postFinal(tx, { businessId, userId, sale: { ...sale, saleNumber }, lines, totals, tenders, amountPaid, amountDue, currency });
}

/** Record a payment against an already-posted invoice: cash in, receivable down. */
async function addPayment(tx, { businessId, userId, currency = 'USD', saleId, payment }) {
  const rows = await tx.$queryRaw`SELECT status, amount_due, customer_id FROM sales WHERE id = ${saleId}::uuid AND business_id = ${businessId}::uuid FOR UPDATE`;
  if (!rows.length) throw bad('Sale not found.', 404);
  const row = rows[0];
  if (NON_POSTING.has(row.status)) throw bad('Finalise the document before taking payment against it.');

  const amount = round2(payment.amount);
  if (amount <= 0) throw bad('Payment amount must be greater than zero.');
  const due = round2(row.amount_due);
  if (amount - due > 0.01) throw bad(`That is more than the ${due.toFixed(2)} still due on this sale.`);
  await assertPaymentAccounts(tx, businessId, [payment]);

  await tx.salePayment.create({
    data: {
      businessId, saleId, provider: payment.method || 'cash', amount,
      currency, status: 'completed', note: payment.note || null,
  ...
  if (row.customer_id) {
    await credit.postRepayment(tx, {
      businessId, customerId: row.customer_id, amount, currency,
      paymentMethod: payment.method || 'cash', description: 'Invoice payment', recordedById: userId,
    });
  }

  await accounting.postSalePayment(tx, {
    businessId, saleId, method: payment.method || 'cash', amount, createdById: userId,
  });

  return tx.sale.update({
    where: { id: saleId },
    data: { amountPaid: { increment: amount }, amountDue: { decrement: amount } },
    include: { payments: true },
  });
}

module.exports = { NON_POSTING, computeTotals, createInvoice, finalizeInvoice, addPayment, mintInvoiceNumber, round2 };
