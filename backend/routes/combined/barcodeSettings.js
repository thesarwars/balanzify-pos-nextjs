...
barcodeSettingsRouter.post('/restore-defaults', auth, requireRole('owner', 'manager'), async (req, res, next) => {
  try {
    const created = await seedBarcodeSettings(prisma, req.user.business_id, req.user.id);
    const settings = await prisma.barcodeSetting.findMany({
      where: { businessId: req.user.business_id },
      orderBy: [{ isDefault: 'desc' }, { name: 'asc' }],
    });
    res.json({ message: created ? `Restored ${created} default sheet${created === 1 ? '' : 's'}.` : 'All default sheets are already present.', created, settings });
  } catch (err) { next(err); }
});

barcodeSettingsRouter.post('/', auth, requireRole('owner', 'manager'), validate(BarcodeSettingSchema), async (req, res, next) => {
  try {
    const data = toBody(req.body);
    const created = await prisma.$transaction(async (tx) => {
      if (data.isDefault) {
        await tx.barcodeSetting.updateMany({ where: { businessId: req.user.business_id, isDefault: true }, data: { isDefault: false } });
      }
      return tx.barcodeSetting.create({ data: { ...data, businessId: req.user.business_id, createdById: req.user.id } });
    });
    res.status(201).json(created);
  } catch (err) { next(err); }
});

barcodeSettingsRouter.put('/:id', auth, requireRole('owner', 'manager'), validate(BarcodeSettingSchema), async (req, res, next) => {
  try {
    const existing = await prisma.barcodeSetting.findUnique({ where: { id: req.params.id }, select: { businessId: true } });
    if (!existing || existing.businessId !== req.user.business_id) return res.status(404).json({ title: 'Not found', status: 404 });

    const data = toBody(req.body);
    const updated = await prisma.$transaction(async (tx) => {
      if (data.isDefault) {
        await tx.barcodeSetting.updateMany({ where: { businessId: req.user.business_id, isDefault: true, id: { not: req.params.id } }, data: { isDefault: false } });
      }
...
barcodeSettingsRouter.delete('/:id', auth, requireRole('owner', 'manager'), async (req, res, next) => {
  try {
    const existing = await prisma.barcodeSetting.findUnique({ where: { id: req.params.id }, select: { businessId: true } });
    if (!existing || existing.businessId !== req.user.business_id) return res.status(404).json({ title: 'Not found', status: 404 });
