...
customersRouter.post('/', auth, validate(CustomerSchema), async (req, res, next) => {
  try {
    ...
    const customer = await prisma.customer.create({ data: { businessId: req.user.business_id, name: req.body.name, phone: req.body.phone, whatsapp: req.body.whatsapp, email: req.body.email, address: req.body.address, creditLimit, customerGroupId: req.body.customer_group_id || null, priceGroupId: req.body.price_group_id || null, ...(req.body.wholesale_terms_days !== undefined && { wholesaleTermsDays: req.body.wholesale_terms_days }), ...(req.body.contact_kind && { contactKind: req.body.contact_kind }), assignedToId: req.body.assigned_to_id || null, ...(req.body.opening_balance !== undefined && { openingBalance: req.body.opening_balance, outstandingBalance: req.body.opening_balance }), notes: req.body.notes } });
